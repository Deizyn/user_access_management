...
import { isSpecialGroup, getGroupBadgeInfo, partitionGroups, getUserPrimaryInternetLevel } from '../../utils/groupHelpers';
...
const getSpecialEntitlementMeta = (name: string) => {
  const lower = name.toLowerCase();
  if (lower.includes('video') || lower.includes('media') || lower.includes('stream')) {
    return { icon: Video, color: 'text-purple-600', label: 'Media & Video Access' };
  }
  if (lower.includes('communication') || lower.includes('chat') || lower.includes('voip') || lower.includes('phone') || lower.includes('messaging')) {
    return { icon: MessageSquare, color: 'text-sky-600', label: 'Communications' };
  }
  if (lower.includes('mail') || lower.includes('email') || lower.includes('webmail')) {
    return { icon: Mail, color: 'text-amber-600', label: 'External Email Privilege' };
  }
  return { icon: Sparkles, color: 'text-indigo-600', label: 'Special Entitlement' };
};
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-3 rounded-lg bg-slate-50 border border-slate-200/80">
                    ...
                    <div className="text-sm font-bold text-slate-900">
                      Level {getUserPrimaryInternetLevel(user.groups, user.internet_level)}
                    </div>
                  </div>
                  ...
                  {/* Special Entitlements in the Same Grid Card Format with Contextual Icons & Colors */}
                  {derivedProfile.specialPermissions.map((perm, idx) => {
                    const meta = getSpecialEntitlementMeta(perm);
                    ...
                    return (
                      <div key={idx} className="p-3 rounded-lg bg-slate-50 border border-slate-200/80">
                        ...
                        <div className="text-sm font-bold text-slate-900">{perm}</div>
                      </div>
                    );
                  })}
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
                    {filteredGroupsList.map((g) => {
                      ...
                      return (
                        <div
                          key={g.group_id}
                          className={`p-3.5 rounded-xl border flex flex-col justify-between transition-all ${isSpec
